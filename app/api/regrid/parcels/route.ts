...
export async function GET(request: Request) {
  ...
  if (lat1 && lng1 && lat2 && lng2) {
    ...
    apiUrl = `https://app.regrid.com/api/v2/parcels/point?lat=${lat}&lon=${lng}&radius=200&limit=5&return_geometry=true&token=${token}`;
  } else {
    return NextResponse.json(
  ...
  try {
    ...
    // For single-point queries, retry with larger radius if empty
    if (lat && lng && !lat1) {
      let data = await res.json();
      let features = data.parcels?.features || data.features || [];

      // Try 500m radius if 200m found nothing
      if (features.length === 0) {
        const retryUrl = `https://app.regrid.com/api/v2/parcels/point?lat=${lat}&lon=${lng}&radius=500&limit=5&return_geometry=true&token=${token}`;
        const retryRes = await fetch(retryUrl);
        if (retryRes.ok) {
          data = await retryRes.json();
          features = data.parcels?.features || data.features || [];
        }
      }
      ...
      return await processFeatures(features);
    }
...
async function processFeatures(features: Record<string, unknown>[]) {
